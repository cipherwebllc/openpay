...
> **語彙について**: 利用規約・特商法表記等の legal 文書では、本セクションで言う **「運営手数料 (1.0% / 純マージン)」+「gas 肩代わり相当 (見積)」** を bundle した総称として **「OpenPay 利用手数料」** を用いています。gas 見積はリスクバッファ込み (実費との差額は精算・返金しない設計) のため、まとめて単一のサービス利用料として扱う方が legal 上のリスク回避が容易だからです。実装上の計算 (1% マージン + gas estimate) と料率は **変更ありません**。

**運営手数料 (1.0% マージン) は常に店主負担** (SaaS / カード決済の販売手数料的な固定コスト、顧客には不可視)。
QR 発行時に店主が **gas 相当額の負担者** を選択 (= OpenPay 利用手数料のうち、gas 肩代わり相当を顧客に上乗せ請求するか、店主が吸収するか):
...
- ☐ **Sentry alert rule の自動セットアップ** — Sentry Dashboard 手動操作は不要。Personal Access Token (project:write + alerts:write scope) を発行後、以下を 1 度実行すれば **5 rule** を冪等作成: `SENTRY_AUTH_TOKEN=... SENTRY_ORG_SLUG=... SENTRY_PROJECT_SLUG=... node scripts/setup-sentry-alerts.mjs`。同名 rule が既にあれば skip。**ルール一覧**: payment.failed (50/h)、smart-account.init-failed (10/h)、x402.middleware.error (10/h)、history.load.invalid-entries-dropped (100/h)、localStorage.set failed (100/h、quota / Safari ITP)。しきい値変更は `scripts/setup-sentry-alerts.mjs` の `RULES.threshold` を編集後、Dashboard で旧 rule を削除して再実行。Slack 通知を追加するには Dashboard で各 rule の Actions に「Send a Slack notification」を追加 (本 script では `NotifyEventAction` で project default 通知設定に従う)
...
- ☐ **`/history` の mobile CSV ダウンロード実機確認** — iOS Safari / Android Chrome で `download` attribute + Blob URL の挙動が browser/OS バージョンで不安定 (ファイルが開けない・"undefined" ファイル名等)。CSV export を mobile で常用する merchant が出た場合、share sheet 経由のフォールバック導線を追加判断。現状は desktop chromium / mobile-safari e2e で empty/seeded 表示のみ smoke 済、download 自体は jsdom mock + RFC-4180 round-trip parser 検証のみ
