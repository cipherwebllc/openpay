...
The `openpay-x402-mcp` package (≥ 0.8.0) has two explicit profiles. `openpay-order-mcp` is the keyless human-pays profile: it exposes only `order_menu`, `order_summary`, and `createOrderLink`, and the customer opens the returned link and pays from their own wallet. Run it with `npx --yes --package=openpay-x402-mcp -- openpay-order-mcp`. The backward-compatible `openpay-x402-mcp` profile keeps all seven tools, including `discovery_search`, `x402_quote`, `x402_pay`, and `order_quote`, for agents that perform guarded autonomous x402 payments.
