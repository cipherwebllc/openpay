...
- Gasless for **JPYC** uses OpenPay's self-hosted **EIP-3009 relayer**: the customer signs a `receiveWithAuthorization` to a forwarder (or, where no forwarder is configured, a plain `transferWithAuthorization` directly to the merchant — a plain EIP-712 signature, **no EIP-7702 delegation required**, so it works in MetaMask and other injected wallets), and the relayer submits it on-chain. In the **recover** configuration (forwarder set), OpenPay sponsors the gas (POL / KAIA) and a per-transaction **OpenPay usage fee** is split to the OpenPay fee-receiver atomically in the same transaction (see Fees); the merchant chooses who bears it via the gas-bearer toggle. Live on **Polygon and Kaia** mainnet. Pimlico / EIP-7702 sponsorship remains as an automatic fallback.
