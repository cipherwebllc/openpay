...
- **License NFTs** *(implementation draft; flags default OFF)* — seller-defined access licenses with ERC-1155 proof on Polygon (Amoy for testing), paid directly to the seller in JPYC. Policy: 1–10,000 units, integer price at least 1,000 JPYC, non-transferable by default; transferable NFTs transfer the license. No usage counts or balances; purchase rights survive mint delay or failure. Seller-funded refunds are separate transfers. `GET /api/license/verify?address=&product=` is a read-only HTTPS status API: unknown is `entitled: null`, not authentication or a signed attestation. SIWE holders can use `/api/store/content/<product>` and discover incoming licenses via `/api/store/library?source=holders` (follow `nextCursor` as `cursor`; the ordinary library remains purchase history). Ledger and Verify data are public. Third-party gating is not guaranteed. See Terms Article 13 and deployment checklist §16.4–16.7; wording requires approval before publication.
...
| `ENABLE_LICENSE_NFT` | Server-only license NFT foundation. Default **off**; requires `ENABLE_CREATOR_STORE`. Disabled licenses are hidden while records, stock holds and mint obligations remain durable. Keep off until all license PRs and Amoy E2E are complete. | optional |
| `NEXT_PUBLIC_ENABLE_LICENSE_NFT` | License UI child flag of `NEXT_PUBLIC_ENABLE_CREATOR_STORE`; default **off**. UI is a separate PR and cannot enable server admission. | optional |
| `LICENSE_NFT_SELLER_ALLOWLIST` | Server-only comma-separated checksummed seller addresses; empty or malformed means nobody may create a license. | optional |
