...
| `UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN` | Upstash Redis **REST 直結** (`https://<db>.upstash.io` + REST token)。設定されていれば `KV_REST_API_*` より優先。本番はこちらを使う (Vercel の Storage 連携が `KV_REST_API_*` を旧プロキシに戻しても影響しない・docs/DEPLOY_CHECKLIST.md §1.1 ④) | optional (production: recommended) |
| `KV_REST_API_URL` / `KV_REST_API_TOKEN` | 互換名 (Vercel Storage 連携が注入)。用途は同じ: payment log **and** SIWE sessions / **encrypted** freee tokens / entitlements / x402 registry。⚠️ 旧 Vercel KV プロキシ (`*.kv.vercel-storage.com`) は使わない (2026-09-03 から EVAL が構文エラーになり再検証 cron が停止した) | optional (required for SIWE + freee sync) |
