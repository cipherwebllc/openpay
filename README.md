...
| `KV_REST_API_URL` / `KV_REST_API_TOKEN` | Upstash Redis **REST 直結** (`https://<db>.upstash.io` + REST token) — payment log **and** SIWE sessions / **encrypted** freee tokens / entitlements / x402 registry。⚠️ 旧 Vercel KV プロキシ (`*.kv.vercel-storage.com`) は使わない (2026-09-03 から EVAL が構文エラーになり再検証 cron が停止した・docs/DEPLOY_CHECKLIST.md §1.1 ④) | optional (required for SIWE + freee sync) |
