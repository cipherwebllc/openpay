...
import { directoryLicenseContentFor, directoryLicenseMetadata } from '@/lib/directory/licenseTerms';

export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  const { locale } = await params;
  setRequestLocale(locale);
  return directoryLicenseMetadata(locale);
}

// Public even when license listing flags are OFF: purchase terms must remain reachable.
export default async function DirectoryLicenseTermsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
