...
export function validLicenseForm(fields: LicenseFormFields, price: string): boolean {
  ...
  return /^[0-9]+$/.test(fields.supply) && Number(fields.supply) >= 1 && Number(fields.supply) <= 10_000 &&
    ...
    isAddress(fields.payTo) && https && fields.termsUrl.length <= 512 && fields.termsVersion.trim().length > 0 && fields.termsVersion.trim().length <= 128;
}
  ...
  return (
    <div className="grid grid-cols-1 gap-4 rounded-xl border border-indigo-200 bg-indigo-50/50 p-4 sm:col-span-2 sm:grid-cols-2">
      ...
      <label className="text-sm font-medium text-slate-800">
        ...
        <input type="text" required maxLength={128} readOnly={readOnly} value={fields.termsVersion} onChange={(e) => onChange({ termsVersion: e.target.value })} className={inputClass} />
      </label>
