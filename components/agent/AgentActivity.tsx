...
function ActivityForAddress({ address, locale, c, refreshKey, chainId, tokenAddress }: Props & { chainId: number; tokenAddress: string }) {
  ...
  function date(item: AgentActivityItem) {
    return <time dateTime={new Date(item.timestamp * 1000).toISOString()}>{formatter.format(item.timestamp * 1000)}</time>;
  }
  function direction(item: AgentActivityItem) {
    const Icon = item.direction === 'in' ? ArrowDownLeft : ArrowUpRight;
    return <span className="inline-flex items-center gap-1"><Icon size={16} aria-hidden />{item.direction === 'in' ? c.filterIn : c.filterOut}</span>;
  }
  ...
  function transaction(item: AgentActivityItem) {
    return <a href={`${explorerBase}/tx/${item.hash}`} target="_blank" rel="noopener noreferrer" className={`inline-block py-2 text-emerald-700 underline ${focus}`}>{c.viewTx}</a>;
  }

  return (
    <section className="mt-6 min-w-0 border-t border-slate-200 pt-5">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-[minmax(0,1fr)_minmax(0,2fr)] sm:items-start">
        ...
        <dl className="grid grid-cols-1 gap-3 text-xs sm:grid-cols-2">
          {([[c.stat24h, 86_400], [c.stat7d, 604_800]] as const).map(([label, windowSec]) => {
            ...
            return <div key={label} className="min-w-0"><dt className="text-slate-500">{label}</dt><dd className="mt-1 break-all font-medium text-slate-900">{stat?.complete ? `${formatJpyc(stat.totalAtomic)} JPYC` : '—'}</dd>{stat && !stat.complete ? <dd className="mt-1 text-slate-500">{c.statsPartial}</dd> : null}</div>;
          })}
        </dl>
      </div>
      <p className="mt-3 text-xs leading-relaxed text-slate-500">{c.publicNote}</p>
      <div className="mt-4 flex flex-wrap gap-2">
        {(['all', 'in', 'out'] as const).map((value) => <button key={value} type="button" aria-pressed={filter === value} className={`min-h-11 rounded-full px-4 py-2 text-sm ${filter === value ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-700'} ${focus}`} onClick={() => { setFilter(value); setVisibleCount(10); }}>{value === 'all' ? c.filterAll : value === 'in' ? c.filterIn : c.filterOut}</button>)}
      </div>
          ...
          {rows.length === 0 ? <p className="mt-4 text-sm text-slate-600">{result.items.length === 0 ? c.empty : c.filterEmpty}</p> : (
            <>
              <table className="mt-4 hidden w-full table-fixed text-left text-xs sm:table">
                <thead className="text-slate-500"><tr>{[c.colDate, c.colType, c.colCounterparty, c.colAmount, c.viewTx].map((label) => <th key={label} scope="col" className="px-2 py-2 font-medium">{label}</th>)}</tr></thead>
                <tbody>{rows.map((item) => <tr key={item.key} className="border-t border-slate-100 align-top"><td className="break-words px-2 py-3">{date(item)}</td><td className="px-2 py-3">{direction(item)}</td><td className="break-words px-2 py-3">{counterparty(item)}</td><td className="px-2 py-3">{amount(item)}</td><td className="break-words px-2 py-1">{transaction(item)}</td></tr>)}</tbody>
              </table>
              <ul className="mt-4 divide-y divide-slate-100 sm:hidden">
                {rows.map((item) => <li key={item.key} className="space-y-2 py-3 text-xs">
                  ...
                  <div className="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1 text-slate-600"><span><span className="sr-only">{c.colDate}: </span>{date(item)}</span><span><span className="sr-only">{c.colCounterparty}: </span>{counterparty(item)}</span>{transaction(item)}</div>
                </li>)}
              </ul>
            </>
          )}
