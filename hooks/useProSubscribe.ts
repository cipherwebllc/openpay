...
type PendingPro = { txHash: Hex; chainId: number; wallet: string };
...
function loadPending(): PendingPro | null {
  try {
    const raw = window.localStorage.getItem(PENDING_KEY);
    if (!raw) return null;
    const o = JSON.parse(raw) as Partial<PendingPro>;
    if (
...
function savePending(rec: PendingPro): void {
  try {
...
export function useProSubscribe(deployment: TokenDeployment) {
  ...
  const [phase, setPhase] = useState<ProSubscribePhase>('idle');
  ...
  const subscribe = useMutation<SubscribeResponse, Error, PendingPro>({
    ...
    onSuccess: () => {
      ...
      setPhase('success');
      void qc.invalidateQueries({ queryKey: ['pro'] });
    },
    onError: (e) => {
  ...
  // 送金開始 (500 JPYC → FEE_RECEIVER)。FEE_RECEIVER 未設定なら呼出側が事前にガードする前提だが、
  // 二重防御でここでも弾く (未設定の宛先へ 500 JPYC を送らせない)。
  const start = useCallback(() => {
    ...
    payWrite.writeContract({
      ...
      args: [env.feeReceiver as Address, proPriceWei],
    });
  }, [address, chainId, deployment.address, payWrite, subscribe]);

  // subscribe のみ再試行 (再送金しない・保持済み or 耐久化された txHash を使う)。
  const retrySubscribe = useCallback(() => {
    const rec: PendingPro | null =
