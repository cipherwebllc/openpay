...
//   (a) JPYC                : gross (saleAmount ?? merchantAmount) を 1:1 → exact。
//   (b) USDC + JPYC anchor  : anchorAmount (= 店主が建てた元の円価格) → exact・レート非依存。
//   (c) USDC・anchor 無し    : gross (saleAmount ?? merchantAmount) * usdcJpy → approx /
//                             (現レート換算・tx 時点ではない) /
//                             レート無しなら unavailable。
//
...
export function entryYenValue(
  ...
  if (entry.asset === 'jpyc') {
    // (a) JPYC は売上総額 (gross) を円ペッグ 1:1。legacy は着金額へ fallback。
    return {
      kind: 'exact',
      yen: roundYen(
        rawToNumber(
          entry.saleAmount ?? entry.merchantAmount,
          HISTORY_ASSET_DECIMALS.jpyc,
        ),
      ),
    };
  }
  ...
  if (typeof usdcJpy === 'number' && Number.isFinite(usdcJpy) && usdcJpy > 0) {
    const usdc = rawToNumber(
      entry.saleAmount ?? entry.merchantAmount,
      HISTORY_ASSET_DECIMALS.usdc,
    );
