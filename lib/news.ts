...
export const NEWS_ITEMS: readonly NewsItem[] = [
  {
    ...
    body: {
      ja: 'Claude や Codex などの AI エージェントを OpenPay につなぐ「Agent」ページを追加しました。接続用のプロンプトをコピーするか、Claude / Codex のデスクトップアプリをそのまま開いて設定できます。支払い用のウォレットは MCP (openpay-x402-mcp 0.15) があなたのマシン上に作り、AI には公開アドレスしか渡りません。OpenPay は鍵を預かりません。ページでは Agent のアドレスの JPYC 残高を確認でき、接続中のウォレットから入金できます（通常の送金・OpenPay の徴収なし）。1 回・累計・1 日の支払い上限は Agent 側の MCP が強制する値で、OpenPay のサーバーが強制するものではありません。AI が支払うときの利用料は従来どおりです。',
      en: 'A new Agent page connects AI agents such as Claude and Codex to OpenPay. Copy a setup prompt, or open the Claude / Codex desktop app directly. The paying wallet is created on your own machine by the MCP (openpay-x402-mcp 0.15) and the AI only ever sees its public address; OpenPay never holds the key. The page shows the agent address\'s JPYC balance and lets you fund it from your connected wallet (a plain transfer; OpenPay collects nothing). Per-call, cumulative and daily spending caps are enforced by the MCP on the agent\'s side, not by OpenPay\'s servers. Usage fees for AI payments are unchanged.',
    },
