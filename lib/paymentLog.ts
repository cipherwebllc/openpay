...
// export / stats が読む単一リスト。route で件数 cap と書込ごとの TTL refresh を適用する。
// 利用者の ipPrefix は保存しない。reader のない日次リストへの複製は廃止。
...
// client (ブラウザ) が log API に申告してよい検証ステータス。'verified' の
// サーバ側 writer は存在しない。旧履歴との型互換のため残すが、client 申告は拒否する。
