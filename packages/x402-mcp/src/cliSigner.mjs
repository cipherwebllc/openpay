...
function childEnvironment(env, excludedEnvPrefixes) {
  ...
  return Object.fromEntries(Object.keys(env)
    .filter((key) => key !== 'BUYER_PRIVATE_KEY' && !excludedEnvPrefixes.some((prefix) => key.startsWith(prefix)))
    .map((key) => [key, env[key]]));
}
...
export function createCliSigner({
  ...
  excludedEnvPrefixes, parseResponse, errors,
}) {
  ...
  return {
    ...
    async signTypedData({ domain, types, primaryType, message }) {
      try {
        ...
        const { error, stdout } = await runCli(execFileImpl, bin, args(typedData, json),
          childEnvironment(env, excludedEnvPrefixes), deadlineMs, errors.failed);
        ...
        const { signature, errorCode } = parseResponse(stdout);
