...
beforeEach(() => { vi.clearAllMocks(); h.auth.mockReturnValue(true); h.enabled = true; });
describe('license mint cron', () => {
  ...
  it.each([{ ok: true, skipped: 'locked', processed: 0, failed: 0, status: 200 }, { ok: false, error: 'storage_unavailable', status: 503 }, { ok: true, processed: 1, failed: 1, status: 503 }])('surfaces lock and storage outcomes: $status', async ({ status, ...result }) => {
    h.run.mockResolvedValue(result); const started = Date.now(); const response = await GET(request()); expect(response.status).toBe(status); expect(await response.json()).toEqual(result); expect(maxDuration).toBe(60); expect(h.run.mock.calls[0]![0].deadline - started).toBeLessThanOrEqual(40_010);
  });
});
