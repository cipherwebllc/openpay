...
const PINS = { resourceId: 'res-abc', expectedRecipient: '0x1111111111111111111111111111111111111111' };
const USDC_PIN = '0x2222222222222222222222222222222222222222';

describe('lib/x402/paywallSnippet', () => {
  const snippet = buildPaywallSnippet('https://api.example.jp/paid/translate', PINS);
  ...
  it('P2-K: 登録 URL の特殊文字が JS へ注入されない (JSON.stringify エスケープ)', () => {
    ...
    const s = buildPaywallSnippet(evil, PINS);
...
describe('lib/x402/paywallSnippet dual-rail (dualRail 指定時)', () => {
  const dual = buildPaywallSnippet('https://api.example.jp/paid/translate', {
    ...PINS, dualRail: true, expectedUsdcRecipient: USDC_PIN,
  });

  it('JPYC 版も出品 ID を固定し、USDC リレーを使用しない', () => {
    const jpycOnly = buildPaywallSnippet('https://api.example.jp/paid/translate', PINS);
    expect(jpycOnly).toContain('export async function jpycGate');
    expect(jpycOnly).not.toContain('/api/x402/relay/');
    expect(jpycOnly).toContain('MY_RESOURCE_ID');
  });
  ...
  it('P2-K 同等: 出品 ID の特殊文字も注入不成立 (JSON.stringify エスケープ)', () => {
    ...
    const s = buildPaywallSnippet('https://x.test/ok', { ...PINS, resourceId: evilId, dualRail: true, expectedUsdcRecipient: USDC_PIN });
