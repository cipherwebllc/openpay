...
import { USDC_DIRECTORY_LICENSED, USDC_DIRECTORY_LIST, USDC_DIRECTORY_SEARCH, USDC_PAYMENT_MONITOR, USDC_SERVICE_MONITOR } from '@/lib/directory/usdcResource';
...
describe('USDC_CATALOG_ITEMS', () => {
  ...
  it('価格・文言は各 endpoint の単一情報源と一致する (ドリフト防止)', () => {
    ...
    for (const r of [USDC_DIRECTORY_LICENSED, USDC_DIRECTORY_LIST, USDC_DIRECTORY_SEARCH, USDC_SERVICE_MONITOR, USDC_PAYMENT_MONITOR, USDC_STORES]) {
      const item = byPath.get(r.path)!;
      expect(item, r.path).toBeDefined();
      expect(item.priceUsd).toBe(r.priceUsd);
      expect(item.description).toBe(r.description);
    }
    expect(USDC_CATALOG_ITEMS).toHaveLength(USDC_JPYC_LIVE_RESOURCES.length + 6);
  });
